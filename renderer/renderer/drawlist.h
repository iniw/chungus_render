...
namespace renderer {
	...
	struct s_draw_list {
		...
		size_t m_size_path; // size of the buffer in bytes
		...
		__forceinline void path_to(s_point&& point) { 
			m_points[m_num_points] = point;
			m_num_points++;
		}

		void path_stroke(s_color&& color);

		void add_polyline(const s_point* points, const size_t num_points, s_color&& color, float thickness);
		void add_rect(s_rect&& rect, s_color&& color);
		void add_triangle(s_point&& point1, s_point&& point2, s_point&& point3, s_color&& color);
	};
